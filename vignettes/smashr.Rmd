...
```{r plots-3, fig.width=5, fig.height=4}
par(mfrow = c(2, 2))
...
```{r plots-4, fig.width=12, fig.height=12, eval=FALSE}
par(mfrow = c(1, 1))
plot(t, mu.t, type = "l")
...
Now for the case when the data are distributed according to a Poisson
Process.
...
as the datasets used in them. The first one is from Fan and Yao
(1998). The simulation is as described in Example 2. To deal with the
fact that the sample size is not a sample size of 2, we first reflect
the right portion of the data about the right endpoint so that it is a
...
We also look at the dataset in Example 1 of Fan and Yao (1998). To
take into account replicates, we take the median of the response at
the same point The results are shown below.
